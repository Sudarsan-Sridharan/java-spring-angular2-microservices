package com.soagrowers.pollcommand;
...
public class PollRestController {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(PollRestController.class);
    ...
    public void add(@PathVariable(value = "id") String id,
        ...
        LOG.debug("Adding Poll [{}] '{}'", id, name);

        try {
            Asserts.INSTANCE.areNotEmpty(Arrays.asList(id, name));
            AddPollCommand command = new AddPollCommand(id, name);
            commandGateway.sendAndWait(command);
            LOG.info("Added Poll [{}] '{}'", id, name);
            ...
            if (null != cex.getCause()) {
                ...
                if (cex.getCause() instanceof ConcurrencyException) {
                    LOG.warn("A duplicate Poll with the same ID [{}] already exists.", id);
